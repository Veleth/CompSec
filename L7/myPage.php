...
class myPage {
	...
	public function adminPanel(){
		$s = "";
		$tmp = file_get_contents("resources/tobeconfirmed.html");
		$db = $this->connect();
		$sql = "SELECT * FROM transactions WHERE `status`='pending'";
		$result = mysqli_query($db, $sql);
		if(mysqli_num_rows($result)==0){
			return "Hello, there are no transactions to be confirmed";
		}
		while($row = $result->fetch_row()){
			...
			$s = str_replace("%5%", $row[5], $s);
			}
		$str = file_get_contents("resources/admin.html");
		$str = str_replace("%tr%", $s, $str);
		return $str;
	}
	...
	public function approve($from, $to, $amo, $ID){
		$db = $this->connect();
		$sql = "UPDATE `transactions` SET `status` = 'confirmed' WHERE `ID` ='".$ID."'";
		mysqli_query($db, $sql);
		$sql2 = "UPDATE `customers` SET `money` = `money` - $amo WHERE `ID` = '".$from."'";
		mysqli_query($db, $sql2);
		$sql3 = "UPDATE `customers` SET `money` = `money` + $amo WHERE `ID` = '".$to."'";
		mysqli_query($db, $sql3);
		$result = mysqli_query($db, $sql);
		return 1;
	}

	public function reject($from, $to, $amo, $ID){
		$db = $this->connect();
	...
	public function getTransactions($acc){
	 ...
	 #[0] - from, [1] - to, [2] - time, [3] - amount, [4] - status, [5] - ID
	 $db = $this->connect();
	 $sql = "SELECT * FROM transactions WHERE (`sender` = '".$acc."' OR `reciever` = '".$acc."') AND `status` = 'confirmed' ORDER BY `time` DESC";
	 $result = mysqli_query($db, $sql);
	 while($row = $result->fetch_row()){
		$s .= $tmp;
		$s = str_replace("%0%", $row[0], $s);
